...
Note: index.md / log.md / overview.md are programmatically appended by Stage 2.6
...
- **`chunk_plan.estimated_total_chunks`** — 1 chunk per 60K chars (ingest.py `target_chars` default). Books with <60K chars of extracted text (e.g. short datasheets, application notes) still get 1 chunk — Stage 1.5 is never skipped.
- **`connections_to_existing_wiki`** (in Stage 1.5 per-chunk analysis, not Stage 1) — be conservative. Only flag clear conflicts, not subtle differences in framing. False positives pollute the LLM-curated review queue.
