...
4 个信号算两个页面的关联度，权重与 improved-wiki graph.py 完全对齐：
...
**无 LLM、无 embedding、无模型调用**。纯文件读写 + 数学计算。improved-wiki 的 graph.py 完全对齐（同权重、同算法）。
...
| keyword 搜索 | 有（CJK bigram + 加权评分） | 无独立 keyword 搜索（靠 search_files grep） |
| vector 搜索 | 有（LanceDB chunk 级） | 有（LanceDB page 级） |
| 融合策略 | RRF (K=60) | 无融合（纯 vector） |
| embedding 默认 | 无默认（用户配远程 API） | 本地 Ollama bge-m3 |
| embedding 后端 | Google/Volcengine/OpenAI 兼容 | OpenAI 兼容（默认 Ollama） |
| 降级策略 | vector 失败 → 纯 keyword | vector 失败 → 报错 |
| chunk 级搜索 | 有（page 聚合 blended） | 无（page 级直接搜） |

**关键差异**：
1. NashSU 的搜索是 **hybrid**（keyword+vector+RRF），improved-wiki 是 **纯 vector**
2. NashSU **不内置任何本地模型**——embedding 完全由用户配置远程 API
3. improved-wiki 默认用本地 Ollama bge-m3 是**自己的添加**，不是 NashSU 的做法
4. NashSU 有 chunk 级向量搜索 + page 聚合，improved-wiki 是 page 级
5. NashSU 的 keyword 搜索有完整的 CJK 分词 + 加权评分体系，improved-wiki 没有
...
- ✅ graph 关联（4 信号权重 + Louvain 社区）— 完全对齐
- ❌ 搜索检索 — **不对齐**（NashSU hybrid+RRF+远程API vs improved-wiki 纯 vector+本地 Ollama）

如果未来要对齐搜索侧，需要：
1. 加 keyword 搜索（CJK bigram + 加权评分）到 search_wiki.py
2. 加 RRF 融合层
3. 支持 chunk 级向量搜索 + page 聚合
4. 让 embedding 后端可配置（不只 Ollama）
