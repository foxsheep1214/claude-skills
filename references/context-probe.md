...
3b. **Known-model validation.** A model self-report is never taken as ground truth on its own — a model can guess confidently *and* wrongly (real cases, both later corrected: Claude Opus 4.8 — genuinely 1M — self-reported a cautious `200000`; Claude Sonnet 5 — also genuinely 1M — self-reported `200000` from a generic "typical model" impression, 2026-07-09). When the model's self-report matches `_KNOWN_MODEL_CONTEXT` (authoritative specs — keep this list synced with the `claude-api` skill's model table whenever a new model shows up in a probe), the probed value is **pinned to the spec** regardless of what the model said. An *unrecognized* model's confident self-report is used but printed with an `⚠️ UNVERIFIED` warning. If the model wrote `UNKNOWN` (or nothing parseable), the fallback is the known spec if recognized, else the codebase's conservative built-in default (`_CONTEXT_SIZE_DEFAULT`, 200K) — never a guess. Matched against the *self-report*, not the env name, which the probe distrusts. The pin/fallback logic runs on a *fresh* probe only; a cache reuse returns the stored value as-is, so an already-wrong cached entry needs `--reprobe` (or a hand-edit) to correct.
