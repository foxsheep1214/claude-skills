# Stage 4.1 · Query Auto-Generation

> **触发**：Stage 2.1（source/concept/entity generation）和 Stage 3.1（写盘）完成后自动执行。
...
```
你现在已经完成了《{title}》的消化分析（Stage 2.1），已经写盘完成。现在基于已落盘的 source/concept/entity 页面。
...
- **no-go**：未输出任何 query block 也未输出 `---QUERIES: 0---` 标记 → Stage 2.2 未完成，重跑
